...
RESOURCE_ROOT_ID=$( aws apigateway get-resources --rest-api-id $API_ID --region ${REGION} )
RESOURCE_ROOT_ID=$( echo ${RESOURCE_ROOT_ID} | jq -r '.items[].id'   )


# 3.2 Create the path for the resource.

RESOURCE_ID=$( aws apigateway create-resource  --rest-api-id ${API_ID} --parent-id ${RESOURCE_ROOT_ID}  --path-part ${ENDPOINT_PATH_PART} --region ${REGION} )
